...
##
## This class is the first object that will receive any Modifiers applied to it 
...
func get_modifier(modifier_data : ModifierData) -> Modifier:
	if modifiers.has(modifier_data):
		return modifiers[modifier_data]
	return null
...
func add_modifier(new_modifier : ModifierData):
	...
	if current_mod:
		...
		modifiers[new_modifier] = Modifier.new(new_modifier, self)
		stack_added = true
...
func remove_modifier(modifier_to_remove : ModifierData):
	if modifier_to_remove not in modifiers:
		print("Modifier to remove [%s] not found" % modifier_to_remove.modifier_name)
		return
	
	var modifier_was_removed : bool = modifiers[modifier_to_remove].remove_stack()
